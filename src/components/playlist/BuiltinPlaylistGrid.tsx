...
export function BuiltinPlaylistGrid({
	...
	return (
		<div ref={ref} className="space-y-4">
			...
			{shouldLoad ? (
				<ul
					className={`grid grid-cols-2 gap-4 transition-opacity duration-300 ${hasScrolled ? "opacity-100" : "opacity-60 hover:opacity-100"}`}
				>
					{isLoading
						? // Show loading skeleton
							Array.from({ length: 4 }).map((_, i) => (
								// biome-ignore lint/suspicious/noArrayIndexKey: Its not real data.
								...
								<li key={p.id}>
									...
									>
										<div className="aspect-video w-full overflow-hidden rounded-md border bg-secondary/40 relative">
											{p.thumbnailUrl ? (
												...
												<Image
													src={p.thumbnailUrl}
													alt="thumbnail"
													className="h-full w-full object-cover scale-110 origin-center group-hover:opacity-90 transition-opacity"
													width={256}
													height={256}
												/>
											) : (
											...
											<div className="absolute top-2 right-2">
												<Badge
													variant="outline"
													className="gap-1 px-2 py-0.5 h-5 bg-green-700/20 border border-green-600 text-green-300"
												>
										...
										<div className="mt-2">
											...
											{p.channelTitle &&
												p.title &&
												p.channelTitle.trim().toLowerCase() !==
